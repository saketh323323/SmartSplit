...
debug(`isReplSet ${isReplSet}`); // @ts-ignore
...
module.exports = async () => {
  ...
  const mongoConfig = {};
  debug(`shouldUseSharedDBForAllJestWorkers: ${(0, _helpers.shouldUseSharedDBForAllJestWorkers)()}`); // if we run one mongodb instance for all tests
  ...
  if ((0, _helpers.shouldUseSharedDBForAllJestWorkers)()) {
    ...
    process.env[mongoURLEnvName] = mongoConfig.mongoUri; // Set reference to mongod in order to close the server during teardown.

    global.__MONGOD__ = mongo;
  }

  mongoConfig.mongoDBName = options.instance.dbName; // Write global config to disk because all tests run in different contexts.
