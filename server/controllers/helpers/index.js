...
function logToFile(data) {
  const logFile = path.join(__dirname, 'heap-logs.txt');
...
exports.simplifyDebts = async function () {
  ...
  logToFile("Initial maxHeapDebt contents: " + JSON.stringify(minHeapDebt.toArray()));
  logToFile("Initial maxHeapCredit contents: " + JSON.stringify(minHeapCredit.toArray()));
  ...
  while (!minHeapDebt.empty() && !minHeapCredit.empty()) {
    ...
    logToFile("credit " + JSON.stringify(smallestCredit));
    logToFile("debit: " + JSON.stringify(smallestDebt));
    ...
    logToFile("added: " + JSON.stringify({
      from: smallestDebt.username,
      to: smallestCredit.username,
      amount: transactionAmount,
    }));
  ...
  logToFile("Array: " + JSON.stringify(optimizedDebts, null, 2));
